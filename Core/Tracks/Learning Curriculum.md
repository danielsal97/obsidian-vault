...
| 1 | [[Engineering/Build Process/1 - Preprocessor\|Preprocessor]] | 30 min | What `#include` does, header guards, macros, why duplicate-include bugs happen |
...
| # | Topic | Est. | What you learn |
|---|---|---|---|
| 5 | [[Engineering/Memory/Process Memory Layout\|Process Memory Layout]] | 45 min | Text, data, BSS, heap, stack — every variable has a home; where LDS's Reactor, workers, and storage live |
| 6 | [[Engineering/Memory/Stack vs Heap\|Stack vs Heap]] | 45 min | Stack: automatic, LIFO, O(1), destroyed on return. Heap: manual, persistent, fragmentation risk |
| 7 | [[Engineering/C/Memory - malloc and free\|malloc and free]] | 45 min | How `malloc` actually works internally — free list, coalescing, hidden header; what `new`/`delete` sit on top of |
...
| # | Topic | Est. | What you learn |
|---|---|---|---|
| 8 | [[Engineering/C/Pointers\|Pointers]] | 1 hr | A pointer IS a memory address. Arithmetic, double pointers, function pointers, const positions, void*, NULL safety |
| 9 | [[Engineering/C/Structs and Unions\|Structs and Unions]] | 30 min | Memory layout, padding, largest-first rule, opaque pointers, unions for type-punning |
| 10 | [[Engineering/C/Bitwise Operations\|Bitwise Operations]] | 45 min | AND/OR/XOR/shift, bit masking, signed vs unsigned rules — how LDS packs protocol fields |
| 11 | [[Engineering/C/Strings\|Strings]] | 45 min | Null terminator, `strncpy` trap, `strcmp` vs `==`, safe buffer handling, `snprintf` |
| 12 | [[Engineering/C/File IO\|File IO]] | 30 min | POSIX `open`/`read`/`write`/`lseek`, `pread`/`pwrite`, `errno`, crash-safe write pattern |
...
| # | Topic | Est. | What you learn |
|---|---|---|---|
| 19 | [[Engineering/C++/RAII\|RAII]] | 45 min | Resource lifetime = object lifetime. Stack unwinding runs destructors. No manual cleanup |
| 20 | [[Engineering/C++/Smart Pointers\|Smart Pointers]] | 45 min | `unique_ptr` = RAII wrapper for heap. `shared_ptr` = ref-counted. `weak_ptr` = non-owning |
| 21 | [[Engineering/C++/Move Semantics\|Move Semantics]] | 45 min | lvalue vs rvalue, `std::move` is just a cast, move constructor nulls the source pointer |

### Type System & Polymorphism
| # | Topic | Est. | What you learn |
|---|---|---|---|
| 22 | [[Engineering/C++/Virtual Functions\|Virtual Functions]] | 45 min | vtable = array of function pointers. `override`, pure virtual, virtual destructor, object slicing |
| 23 | [[Engineering/C++/Inheritance\|Inheritance]] | 45 min | Access specifiers, construction order, multiple inheritance, diamond problem, when to use composition |
| 24 | [[Engineering/C++/Templates\|Templates]] | 45 min | Compile-time polymorphism, instantiation in headers, SFINAE, type traits — `Dispatcher<T>` in LDS |
| 25 | [[Engineering/C++/STL Containers\|STL Containers]] | 45 min | `vector`, `map`, `unordered_map`, `priority_queue` — iterator invalidation, complexity guarantees |
| 26 | [[Engineering/C++/Exception Handling\|Exception Handling]] | 45 min | `try`/`catch`/`throw`, exception hierarchy, `noexcept`, stack unwinding fires RAII destructors |
...
| # | Topic | Est. | What you learn |
|---|---|---|---|
| 30 | [[Engineering/Linux/Threads - pthreads\|Threads — pthreads]] | 1 hr | `pthread_create`, mutex, condition variable (why `while` loop), rwlock, TLS, deadlock patterns |
| 31 | [[Engineering/Concurrency/Multithreading Patterns\|Multithreading Patterns]] | 1 hr | Thread pool, producer/consumer, lock hierarchy, futures — how LDS's ThreadPool + WPQ works |
| 32 | [[Engineering/Concurrency/Memory Ordering\|Memory Ordering]] | 1 hr | Happens-before, `std::atomic`, acquire/release, CAS, false sharing, why `volatile` is wrong here |
| 33 | [[Engineering/Linux/Shared Memory\|Shared Memory]] | 45 min | `shm_open`, `mmap MAP_SHARED`, sync requirement — and why LDS uses sockets instead |
| 34 | [[Engineering/Linux/Semaphores\|Semaphores]] | 45 min | Counting semaphore, `sem_timedwait`, producer/consumer — watchdog heartbeat pattern |
| 35 | [[Engineering/Linux/mmap\|mmap]] | 45 min | File-backed mapping, anonymous mapping, `MAP_PRIVATE`, demand paging, `msync` — alternative to `pread`/`pwrite` |
...
| # | Topic | Est. | What you learn |
|---|---|---|---|
| 36 | [[Engineering/Networking/Overview\|Networking Overview]] | 1 hr | Physical → Ethernet → IP → TCP/UDP → TLS → HTTP — full bottom-up OSI walk |
| 37 | [[Engineering/Networking/Sockets TCP\|Sockets TCP]] | 1 hr | `socket`/`bind`/`listen`/`accept`, `RecvAll` loop, `TCP_NODELAY`, byte ordering — `TCPDriverComm` in LDS |
| 38 | [[Engineering/Networking/UDP Sockets\|UDP Sockets]] | 45 min | No connection, message boundaries, MTU, `MSG_ID` tracking — master↔minion protocol in LDS |
| 39 | [[Engineering/Networking/epoll\|epoll]] | 1 hr | `epoll_create`/`epoll_ctl`/`epoll_wait`, level vs edge triggered, O(1) — the LDS Reactor event loop |
| 40 | [[Engineering/Networking/IPC Overview\|IPC Overview]] | 45 min | All IPC mechanisms compared: pipe, socketpair, unix socket, shared mem, mq — when to use which |
...
| # | Topic | Est. | What you learn |
|---|---|---|---|
| 41 | [[Engineering/Design Patterns/Reactor\|Reactor]] | 1 hr | Event loop on top of epoll — single thread, all I/O, handler dispatch. You built this |
| 42 | [[Engineering/Design Patterns/Observer\|Observer]] | 45 min | Subject notifies observers without knowing who they are — `Dispatcher<T>`/`CallBack<T>` in LDS |
| 43 | [[Engineering/Design Patterns/Command\|Command]] | 45 min | Encapsulate a request as an object — `ICommand` + priority queue in LDS |
